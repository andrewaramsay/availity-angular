(function(root) {
  ...
  availity.ui.constant('avCalendarConfig', {
    TEMPLATE: 'ui/daterangepicker/calendar-tpl-new.html'
  });
  ...
  function avCalendarDirective(avCalendarConfig) {
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        _buildMonthRight(scope, start, scope.month.month(scope.month.month()+1));
        ...
        scope.next = function() {
          ...
          _buildMonthRight(scope, next, scope.month.month()+1);
        };
        ...
        scope.previous = function() {
          ...
          _buildMonthRight(scope, previous, scope.month.month()+1);
        };
      }
    };
    ...
    function _buildMonthRight(scope, start, month) {
      scope.weeksRight = [];
      var done = false, date = start.clone(), monthIndex = date.month()+1, count = 0;
      while (!done) {
