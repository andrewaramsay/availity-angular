(function(root) {
  ...
  availity.ui.constant('avDateRangeConfig', {
    ...
    ADD_ON_SELECTOR: '[data-toggle="daterangepicker"]',
  ...
  avDateRangeController.$inject = ['$document', '$scope', '$element', '$attrs', '$templateCache', 'avDateRangeConfig'];
  function avDateRangeController($document, $scope, $element, $attrs, $templateCache, avDateRangeConfig, options) {
    ...
    options.template = $templateCache.get(avDateRangeConfig.TEMPLATE);
    ...
    this.viewToModel = function() {
      var format =  self.options.format; //$element.daterangepicker.parseFormat(self.options.format);
      var utcDate = self.ngModel.$viewValue;
      //var utcDate = $element.daterangepicker.parseDate(self.ngModel.$viewValue, format, 'en');
    ...
    // Fake placeholder function getStartDate
    ...
    //binding functions to daterangepicker in angular

    $element.daterangepicker = function(options, callback) {
      this.each(function() {
        var el = $(this);
        if (el.data('daterangepicker'))
          el.data('daterangepicker').remove();
        el.data('daterangepicker', proto);
      });
      return this;
    };
    ...
    this.container.find('.calendar')
      .on('click.daterangepicker', '.prev', proto.clickPrev)
      .on('click.daterangepicker', '.next', proto.clickNext)
      .on('click.daterangepicker', 'td.available', proto.clickDate)
      .on('mouseenter.daterangepicker', 'td.available', proto.hoverDate)
      .on('mouseleave.daterangepicker', 'td.available', proto.updateFormInputs)
      .on('change.daterangepicker', 'select.yearselect', proto.monthOrYearChanged)
      .on('change.daterangepicker', 'select.monthselect', proto.monthOrYearChanged)
      .on('change.daterangepicker', 'select.hourselect,select.minuteselect,select.secondselect,select.ampmselect', proto.timeChanged)
      .on('click.daterangepicker', '.daterangepicker_input input', proto.showCalendars)
      //.on('keyup.daterangepicker', '.daterangepicker_input input', $.proxy(this.formInputsChanged, this))
      .on('change.daterangepicker', '.daterangepicker_input input', proto.formInputsChanged);

    this.container.find('.ranges')
      .on('click.daterangepicker', 'button.applyBtn', proto.clickApply)
      .on('click.daterangepicker', 'button.cancelBtn', proto.clickCancel)
      .on('click.daterangepicker', 'li', proto.clickRange)
      .on('mouseenter.daterangepicker', 'li', proto.hoverRange)
      .on('mouseleave.daterangepicker', 'li', proto.updateFormInputs);

    if (this.element.is('input')) {
      this.element.on({
        'click.daterangepicker': proto.show,
        'focus.daterangepicker': proto.show,
        'keyup.daterangepicker': proto.controlChanged,
        'keydown.daterangepicker': proto.keydown
      });
    } else {
      this.element.on('click.daterangepicker', proto.toggle);
    }
